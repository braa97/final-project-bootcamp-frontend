...
const Widgets = () => {
  ...
  useEffect(() => {
    ...
    let fetchResidents = async () => {
      let residents;
      if (user?.userType === "Instructor") {
        residents = await apiManager.getResidentsByInstructorId(user?.userId);
      } else {
  ...
  const getCurrentMonthBirthdays = () => {
    ...
    for (let resident of residents) {
      birthMonths.push(utility.dateFormatter(resident.dateOfBirth).slice(3, 5));
    }
  ...
  return (
    <div className="home-widgets-container">
          ...
          {residents.slice(0, 3).map((resident, i) => {
            return (
              <img
                ...
                alt="avatar"
                onClick={() =>
                  navigate(`apartments/apartment-info/resident/${resident._id}`)
                }
              />
            );
          })}
      ...
      <div className={`box ${user?.userType === "Instructor" ? "box4" : "box5"}`}>
        <div>
      ...
      {user?.userType === "Instructor" ? (
        <>
          <div className="box box5">
            <div>
              <h3>{totalMeetingsState ? totalMeetingsState : null} Meetings</h3>
